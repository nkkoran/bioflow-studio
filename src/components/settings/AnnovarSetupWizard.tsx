...
export function AnnovarSetupWizard({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Dialog open={open} onClose={onClose} title="ANNOVAR setup" width="max-w-2xl">
      <div className="flex flex-col gap-3 text-xs text-text-secondary">
        ...
        <div className="rounded border border-border bg-bg-primary p-3 font-mono text-[11px] text-text-primary">
          <div>Scripts: {scriptsPath}</div>
          <div>Databases: {dbPath}</div>
        </div>
        <pre className="max-h-44 overflow-auto rounded border border-border bg-black/30 p-3 text-[11px] text-text-primary">{installCommand}</pre>
        <div className="flex items-center gap-2">
          <Button variant="primary" size="sm" disabled={running} onClick={() => void runSetup()}>
